...
export const TanScene: React.FC<TanSceneProps> = ({ mode = 'story', val1, val2, onParam1Change, onParam2Change }) => {
  return (
    <FormulaLayout
      story={storyData}
      mode={mode}
    ...
    return (
      <group position={[0, 0, 0]}>
        <mesh position={[0, 0.8, 0]} rotation={[0, 0, -angleRad]}>
        ...
        {[-0.6, -0.2, 0.2, 0.6].map((x, idx) => {
          ...
          const h = Math.max(0.4, 1.5 - deltaH);
          return (
            <mesh key={idx} position={[x, -0.8 + h / 2, 0]}>
              <boxGeometry args={[0.07, h, 0.85]} />
